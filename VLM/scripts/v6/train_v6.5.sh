...
LOG_DIR=/scratch/li96/zl9731/cs16/Model/output/VLM/v6.5
mkdir -p "$LOG_DIR"
...
${TORCHRUN} --nproc_per_node=${NPROC_PER_NODE} /scratch/li96/zl9731/cs16/vri-food/VLM/train/trainer.py \
  ...
  --val_dataset $PBS_JOBFS/data/vlm_val_A_v6.jsonl \
  ...
  --init_from_checkpoint /scratch/li96/zl9731/cs16/Model/output/VLM/v6.4 \
  --save_lora_only \
  --lora_r 32 \
  --lora_alpha 64 \
  --lora_dropout 0.05 \
  ...
  --save_steps 4000 \
  --save_total_limit 3 \
  --lambda_lm 1.0 --lambda_lm_title 0.01 --lambda_lm_ing 1.0 \
  --lambda_cuisine 0.0 --lambda_meal 0.0 --lambda_dish 0.0 \
  --lambda_amount 0.3 --lambda_ratio 0.4 --lambda_hinge 0.3 --lambda_total_weight 0.0 \
